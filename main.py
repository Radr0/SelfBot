...
# ctypes.windll.kernel32.SetConsoleTitleW(
    # f'[DIGO SelfBot v{SELFBOT.__version__}] | Загрузка...')
...
[ {ctx.prefix}clear <кол-во> ] - удаляет сообщения.
...
[ {ctx.prefix}webspam<webhook>|<кол-во>|<текст>] - спам вебхуками
[ {ctx.prefix}spamh] - спам
```
**[Наш сервер](https://discord.gg/9haG6m4h7p)**
```css
# radro#2114
...
async def nick(ctx):
    ...
    for member in ctx.guild.members:
        try:
            await member.edit(nick="раб радро2114")
        except:
            pass
...
async def reacts(ctx, count=None, reaction=None):
    ...
    if count == None or reaction == None:
        ...
        embed=discord.Embed(
            title="DIGO", description=f"Вы не выбрали кол-во спама или эмодзи.\n{prefix.strip()}spamreact [кол-во, эмодзи]", color=randcolor)
...
async def dick(ctx, *, user: discord.Member=None):  # b'\xfc
    await ctx.message.delete()
    topchels = ["746669749773271100", "684461165145096433", "881987457715949619"]
    if user is None:
        user=ctx.author
...
async def spamh(ctx, *, hook):
    ...
    fuck=True
    while fuck:
        requests.post(hook, data={'content': "@everyone https://discord.gg/9haG6m4h7p"})
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(res['url']) as resp:
                image=await resp.read()
        with io.BytesIO(image) as file:
            await ctx.send(file=discord.File(file, f"DIGO_anal.gif"))
    except:
        em=discord.Embed()
        em.set_image(url=res['url'])
        await ctx.send(embed=em)
